...
class Road:
    ...
    def __init__(self, start: Tuple[int, int], end: Tuple[int, int], scale: int = 1):
        self._start = start
        self._end = end
        self._type = "blue"
    ...
    def compute_road_position(self, tc, obstaclesList):
        ...
        control_length = sqrt((xc - x1) ** 2 + (yc - y1) ** 2) + sqrt(
            (x2 - xc) ** 2 + (y2 - yc) ** 2
        )
        ...
        for road_index in range(road_num):
            ...
            rotate_factor = commonMath.ang(
                [[xrmp, yrmp], [xrmn, yrmn]], [[0, 0], [1, 0]]
            )
            if yrmp > yrmn:
                rotate_factor = commonMath.ang(
                    [[xrmp, yrmp], [xrmn, yrmn]], [[0, 0], [-1, 0]]
        ...
        for other_road in obstaclesList._stack:
            dist = -1 / (
                1
                + sqrt(
                    (other_road._center[0] - xrm) ** 2
                    + (other_road._center[1] - yrm) ** 2
                )
            )
            distance_num += dist
